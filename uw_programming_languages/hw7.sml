...
fun eval_prog (e,env) =
	...
	(case List.find (fn (s2,v) => s=s2) env of
      ...
      | Shift(x,y,e) => let 
      						val tmp = eval_prog(e,env)
      					in
      						case tmp of
      							NoPoints => tmp
      							|Point(m,n) => Point(m+x,n+y)
      							|Line(m,n) => Line(m,n+y-m*x)
      							|VerticalLine(m) => VerticalLine(m+x)
      							|LineSegment(m,n,p,q) => LineSegment(m+x,n+y,p+x,q+y)
...
fun preprocess_prog e = 
	let 
		fun temp xs =
			let val LineSegment(m,n,p,q) = xs
			in
				if real_close_point (m,n) (p,q) 
					then Point(m,n)
					else if real_close(m,p) andalso n < q then e 
						else if m < p andalso n < q then e else LineSegment(p,q,m,n)

			end
	in
		case e of
			LineSegment(m,n,p,q) => temp e
